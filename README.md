...
`export` / `stats` / `describe` / `filter-ids`（隐藏）/ `serve` & `mcp`（隐藏，暂未实现）。
...
# 8. 预检写入/联网命令，不创建索引、不读输入、不调用 embedding
turbovec-rs --dry-run init --db /tmp/docs.tvim
turbovec-rs --dryrun search --db /tmp/docs.tvim --query "什么是编程"
```
...
`--dryrun`（别名 `--dry-run`）输出 JSON plan。它会解析配置和轻量 JSON 参数，
但不会创建/修改 `.tvim`、SQLite sidecar 或 meta 文件，不会读取 import 输入，
不会读取 `--vector-file`，也不会调用 embedding provider。
